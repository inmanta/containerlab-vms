...
from clab_vm_startup.host.host import CPUVendor, Host
...
class XRV9K_CLI(VirtualRouter):
    ...
    def boot_args(self) -> List[str]:
        ...
        # When using xrv9k on an amd host, we face some issues, this workaround force the usage of
        # simpler emulated cores.
        if self.host.get_cpu_vendor() == CPUVendor.AMD:
            cpu_option = args.index("-cpu")
            if cpu_option >= 0:
                args.pop(cpu_option)  # Removing option marker
                args.pop(cpu_option)  # Removing option argument
            args.extend(["-cpu", "qemu64,+ssse3,+sse4.1,+sse4.2"])
